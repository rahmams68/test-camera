...
function drawMesh(faces, ctx) {
    ...
    if (faces.length > 0) {
        faces.forEach(face => {
            ...
            const keypoints = face.scaledMesh //REAL SIZE LANDMARK
            // const keypoints = face.mesh //MINI SIZE LANDMARK

            //Gambar titik landmark
            for (let i = 0; i < keypoints.length; i++) {
                const [x, y, z] = keypoints[i]
                // console.log(`Keypoints ${i}: ${x}, ${y}, ${z}`)
